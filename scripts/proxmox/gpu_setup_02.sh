...
# Validate and install kernel headers
validate_kernel_headers() {
    ACTIVE_KERNEL=$(uname -r)
    echo "Checking kernel headers for $ACTIVE_KERNEL..."
...
setup_kernel_module() {
    ...
    echo "Ensuring amdgpu kernel module files exist..."
    if [[ ! -f /lib/modules/$(uname -r)/updates/dkms/amdgpu.ko ]]; then
        echo "Reconfiguring amdgpu-dkms..."
...
verify_device_nodes() {
    ...
    if [[ -e /dev/kfd && -d /dev/dri ]]; then
        ...
        echo "GPU device nodes are missing. Reloading GPU modules..."
        sudo modprobe -r amdgpu
        sudo modprobe amdgpu
        if [[ ! -e /dev/kfd || ! -d /dev/dri ]]; then
            echo "Failed to create GPU device nodes. Ensure amdgpu is loaded and firmware is correct."
            exit 1
        fi
    fi
}

# Install vendor-reset for GPU resets
install_vendor_reset() {
    echo "Installing vendor-reset module to handle GPU resets..."
    validate_kernel_headers

    if [[ -d /usr/src/vendor-reset-* ]]; then
        echo "vendor-reset module is already installed. Rebuilding to ensure compatibility..."
        sudo dkms remove vendor-reset --all || true
        sudo dkms add /usr/src/vendor-reset-*
        sudo dkms build vendor-reset
        sudo dkms install vendor-reset
    else
        echo "Cloning vendor-reset repository..."
        git clone https://github.com/gnif/vendor-reset.git /tmp/vendor-reset
        sudo dkms add /tmp/vendor-reset
        sudo dkms build vendor-reset
        sudo dkms install vendor-reset
